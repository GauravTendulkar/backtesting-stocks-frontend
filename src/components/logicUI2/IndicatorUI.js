...
const renderComponent = (currentComponentState, onchange) => {
    ...
    switch (getVal(0)) {
        ...
        case 'countPrevTradesWeekly': return wrap(PrevTradeCount)
        case 'countPrevTradesMonthly': return wrap(PrevTradeCount)
...
const IndicatorUI = ({ indexProp, valueProp, objectPath = [], dispachStateEquation, onChangeProp = () => { }, objectProp = "", removeIndicatorProp = () => { } }) => {

    // console.log("IndicatorUI", objectPath)
    ...
    const onchange = useCallback((data) => {
        // console.log("onChange_IndicatorUI", data, [...objectPath, objectProp])
        dispachStateEquation({ type: "onChange_IndicatorUI", path: [...objectPath, objectProp], value: data })

    }, [objectProp, objectPath])
    ...
    return (
        ...
        >
            <button
                ...
                onClick={() => {
                    // console.log("valueProp[objectProp]", valueProp[objectProp])
                    setCopyIndicator(valueProp[objectProp])
                }//handleCopy
            ...
            <button
                ...
                onClick={() => {
                    console.log("removeIndicator", objectPath)
                    dispachStateEquation({ type: "removeIndicator_IndicatorUI", path: objectPath })
                    // removeIndicatorProp(indexProp)
                }}
                title="Remove"
            >
                <X size={14} />
            ...
            <div className="w-full">{renderComponent(valueProp[objectProp], onchange)}</div>
        </div>
    )
}
